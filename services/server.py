...
class services():
    ...
    def create_solicitud(self, nombre,
        ...
        with connection.cursor() as cursor:
            # Create a new record
            try:
                sql = "INSERT INTO scholmagia (nombre, \
                    apellido, identificacion, \
                        edad, afinidad, estatus,\
                              grimorio) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                cursor.execute(
                    sql, (nombre, apellido, identificacion,
                          edad, afinidad, estatus, grimorio))
            except:
    ...
    def updateSolicitud(self, id, nombre,
        ...
        with connection.cursor() as cursor:
            # Create a new record
            try:
                cursor.execute(('''UPDATE scholmagia SET nombre=('{0}'),\
                    apellido=('{1}'),identificacion=('{2}'),edad=('{3}'),afinidad=('{4}')
                                     WHERE id=('{5}')'''
                                .format(nombre, apellido, identificacion, edad, afinidad, id)))
